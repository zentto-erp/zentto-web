...
| G-05 | Ninguna vertical hermana lee entitlements del core — cada una solo valida auth | ~~**HIGH**~~ **RESUELTO** (Lote 2.D + 3.A) | hotel, medical, tickets, education, rental, inmobiliario | 6 apps adoptan `@zentto/platform-client@^0.6.0` con `checkPlatformConnectivityAtStartup` + `requireEntitlement()` factory. PRs: `zentto-hotel#74`, `zentto-medical#83`, `zentto-tickets#68`, `zentto-education#51`, `zentto-rental#59`, `zentto-inmobiliario#34`. Pendientes: `zentto-pos` y `zentto-restaurante` son frontend-only; `zentto-broker` es MSSQL (fuera de scope). |
...
| G-08 | `/v1/status` sin dimensión por-tenant ni por-app; dashboard ops de Fase 2 no existe | ~~**HIGH**~~ **PARCIAL** (PR #504, Lote 2.E v1) | Stream B | `/v1/status?tenant=<subdomain>` agrega dimensión tenant. Pendiente v2: SP dedicado con métricas + tags observability + dashboard UI en `apps/panel`. |
