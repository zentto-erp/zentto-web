...
export default function CxpMasterPage() {
  ...
  return (
    <Box sx={{ flex: 1, display: "flex", flexDirection: "column", gap: 2, minHeight: 0 }}>
      {/* Seccion superior: Lista de Proveedores */}
      <Paper sx={{ p: 2 }}>
        ...
        <TableBody>
          {documentos.map((d, i) => (
            <TableRow key={`${d.tipoDoc}-${d.numDoc}-${i}`} hover>
              ...
              <TableCell>{d.fecha ? formatDate(d.fecha, { timeZone }) : ""}</TableCell>
          ...
          {rows.length === 0 ? (
            ...
            rows.map((r, i) => (
              <TableRow key={`${r.tipoDoc}-${r.numDoc}-${i}`} hover selected={r.checked}>
                ...
                <TableCell sx={{ fontSize: "0.82rem" }}>{r.fecha ? formatDate(r.fecha, { timeZone }) : ""}</TableCell>
