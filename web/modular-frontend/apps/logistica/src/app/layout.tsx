...
function AppContent({ children }: { children: React.ReactNode }) {
    ...
    // Hidratar configuracion general + logistica desde BD al arrancar
    useHydrateModuleSettings('logistica', company?.companyId ?? 1);
    ...
    return (
        <AppBarWrapper>
            {!showContent ? (
                ...
                <ToastProvider>
                    <ZenttoLayout navigationFields={navigation} appCode="logistica">
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
    return (
        <html lang="es" data-scroll-behavior="smooth" suppressHydrationWarning>
            <head>
                <title>Logistica - Zentto App</title>
                <InitColorSchemeScript attribute="data-toolpad-color-scheme" />
            </head>
            <body>
                <SessionProvider basePath="/logistica/api/auth">
                    <QueryProvider>
                        <AuthProvider>
