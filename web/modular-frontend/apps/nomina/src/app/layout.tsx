...
import { buildNominaNav } from './nav';

function AppContent({ children }: { children: React.ReactNode }) {
    const { isLoading, isAdmin, modulos, company } = useAuth();
    const [showContent, setShowContent] = useState(false);

    // Hidratar configuración general + nómina desde BD al arrancar
    useHydrateModuleSettings('nomina', company?.companyId ?? 1);
    ...
    const navigation = React.useMemo(() => buildNominaNav(isAdmin, modulos), [isAdmin, modulos]);
    ...
    return (
        <AppBarWrapper>
            {!showContent ? (
                ...
                <ToastProvider>
                    <ZenttoLayout navigationFields={navigation} appCode="nomina">
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
    return (
        <html lang="es" data-scroll-behavior="smooth" suppressHydrationWarning>
            <head>
                <title>Nómina - Zentto App</title>
                <InitColorSchemeScript attribute="data-toolpad-color-scheme" />
            </head>
            <body>
                <SessionProvider basePath="/nomina/api/auth">
                    <QueryProvider>
                        <AuthProvider>
