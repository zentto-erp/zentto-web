...
const GRID_ID = buildEcommerceGridId('admin-marcas', 'main');
...
export default function AdminMarcasPage() {
    ...
    const upsertMut = useUpsertBrand();
    ...
    useEffect(() => {
        ...
        const onAction = (e: any) => {
            ...
            if (action === 'edit') {
                ...
                if (window.confirm(`¿Eliminar marca "${row.name}"?`)) {
                    deleteMut.mutate(row.code);
                }
            }
        };
        const onCreate = () => { setForm({ code: '', name: '', description: '' }); setOpen(true); };
    ...
    return (
        <Box>
            ...
            <zentto-grid
                ref={gridRef}
                export-filename="admin-marcas"
                height="calc(100vh - 240px)"
                enable-create create-label="Nueva marca"
            ...
            <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
                <DialogTitle>{form.isUpdate ? 'Editar marca' : 'Nueva marca'}</DialogTitle>
                <DialogContent>
