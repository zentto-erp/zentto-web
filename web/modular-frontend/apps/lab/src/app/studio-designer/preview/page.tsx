...
export default function PreviewPage() {
  ...
  useEffect(() => {
    try { setConfig(JSON.parse(localStorage.getItem(WIZARD_CONFIG_KEY) || "null")); } catch { /* noop */ }
  ...
  if (!config) {
    return (
      <Box sx={{ p: 4, textAlign: "center" }}>
        <Typography variant="h6" color="text.secondary">No hay configuración para preview.</Typography>
        <Button sx={{ mt: 2 }} onClick={() => router.push("/studio-designer/wizard")}>Ir al Wizard</Button>
      </Box>
    );
  }
    ...
    <Box sx={{ display: "flex", flexDirection: "column", height: "calc(100vh - 64px)" }}>
      <AppBar position="static" color="default" elevation={1}>
        <Toolbar variant="dense" sx={{ gap: 1 }}>
          ...
          <Box flex={1} />
          <Button size="small" startIcon={<CopyIcon />} onClick={() => navigator.clipboard.writeText(JSON.stringify(config, null, 2))}>Copiar JSON</Button>
        </Toolbar>
      </AppBar>
