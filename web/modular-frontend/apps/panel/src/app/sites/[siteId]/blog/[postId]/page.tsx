...
export default function EditPostPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      postsApi.get(siteId, postId),
      categoriesApi.list(siteId).catch(() => []),
      tagsApi.list(siteId).catch(() => []),
    ])
      .then(([post, cats, tags]) => {
        setTitle(post.title || "");
        setSlug(post.slug || "");
        ...
        setSelectedTags(
          Array.isArray(post.tags)
            ? post.tags.map((t: any) => (typeof t === "string" ? t : t.name || t.id))
            : []
        );
        ...
        setAvailableTags(Array.isArray(tags) ? tags : tags?.data ?? []);
      })
  ...
  const handleSave = async (publishNow?: boolean) => {
    ...
    try {
      ...
      await postsApi.update(siteId, postId, payload);
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1200, mx: "auto" }}>
      ...
      <Breadcrumbs sx={{ mb: 2 }}>
        ...
        <Typography color="text.primary">Editar Post</Typography>
      </Breadcrumbs>
      ...
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 3, flexWrap: "wrap", gap: 2 }}>
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          ...
          <Button
            ...
            component="a"
            href={`/sites/${siteId}/blog/${postId}/preview`}
            target="_blank"
          >
            Vista previa
          </Button>
