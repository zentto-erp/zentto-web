...
function AppContent({ children }: { children: React.ReactNode }) {
    ...
    // Hidratar configuración general + inventario desde BD al arrancar
    useHydrateModuleSettings('inventario', company?.companyId ?? 1);
    ...
    return (
        <AppBarWrapper>
            {!showContent ? (
                ...
                <ToastProvider>
                    <ZenttoLayout navigationFields={navigation} appCode="inventario">
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
    return (
        <html lang="es" data-scroll-behavior="smooth" suppressHydrationWarning>
            <head>
                <title>Inventario - Zentto App</title>
                <InitColorSchemeScript attribute="data-toolpad-color-scheme" />
            </head>
            <body>
                <SessionProvider basePath="/inventario/api/auth">
                    <QueryProvider>
                        <AuthProvider>
