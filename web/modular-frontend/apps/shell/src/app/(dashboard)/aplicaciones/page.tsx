...
const CATALOG: StoreApp[] = [
    ...
    { id: 'tickets', name: 'Zentto Tickets', description: 'Eventos, boletos y experiencias con mapas interactivos de asientos.', fullDescription: 'Gestiona venues, eventos, venta de boletos con mapas interactivos de asientos, carreras de calle, inscripciones, dorsales y escaneo QR en puerta. Comisión por boleto vendido.', icon: <ConfirmationNumberIcon sx={{ fontSize: 28 }} />, bgColor: '#6366F1', category: 'Especializados', path: process.env.NEXT_PUBLIC_TICKETS_URL || 'https://tickets.zentto.net', features: ['Mapas de asientos interactivos', 'Editor visual de venues', 'Carreras 5K/10K/maratón', 'QR tickets anti-fraude', 'Hold temporal 10 min', 'Clasificación en tiempo real'], version: '0.1.0', author: 'Zentto' },
    { id: 'medical', name: 'Zentto Medical', description: 'Gestión médica: citas, pacientes, médicos, recetas y chat.', fullDescription: 'Plataforma completa de gestión médica para doctores y clínicas. Agende citas con lifecycle completo, gestione pacientes con historial médico, emita recetas electrónicas, chat médico-paciente y facturación con comisiones.', icon: <LocalHospitalIcon sx={{ fontSize: 28 }} />, bgColor: '#059669', category: 'Especializados', path: process.env.NEXT_PUBLIC_MEDICAL_URL || 'https://medical.zentto.net', features: ['Citas con lifecycle completo', 'Historial médico', 'Recetas electrónicas', 'Chat médico-paciente', 'Dashboard por rol', '15 especialidades médicas'], version: '0.1.0', author: 'Zentto' },
...
export default function AppsStorePage() {
    ...
    const handleOpenApp = (app: StoreApp) => {
        ...
        // URLs absolutas (apps externas como tickets, medical) → abrir directo
