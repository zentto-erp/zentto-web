...
retencionesRouter.get("/", async (req, res) => { const q = qSchema.safeParse(req.query); if (!q.success) return res.status(400).json({ error: "invalid_query" }); res.json(await listRetenciones(q.data)); });
retencionesRouter.get("/:codigo", async (req, res) => { const row = await getRetencion(req.params.codigo); if (!row) return res.status(404).json({ error: "not_found" }); res.json(row); });
