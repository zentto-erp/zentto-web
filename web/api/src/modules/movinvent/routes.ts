...
movInventRouter.get("/", async (req, res) => { const q = qSchema.safeParse(req.query); if (!q.success) return res.status(400).json({ error: "invalid_query" }); res.json(await listMovInvent(q.data)); });
movInventRouter.get("/mes/list", async (req, res) => { const q = qMesSchema.safeParse(req.query); if (!q.success) return res.status(400).json({ error: "invalid_query" }); res.json(await listMovInventMes(q.data)); });
...
movInventRouter.get("/:id", async (req, res) => { const row = await getMovInvent(req.params.id); if (!row) return res.status(404).json({ error: "not_found" }); res.json(row); });
