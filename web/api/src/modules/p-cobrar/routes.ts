...
pCobrarRouter.get("/", async (req, res) => { const q = qSchema.safeParse(req.query); if (!q.success) return res.status(400).json({ error: "invalid_query" }); res.json(await pCobrarService.list(q.data)); });
pCobrarRouter.get("/c/list", async (req, res) => { const q = qSchema.safeParse(req.query); if (!q.success) return res.status(400).json({ error: "invalid_query" }); res.json(await pCobrarService.listC(q.data)); });
...
pCobrarRouter.get("/c/:id", async (req, res) => { const row = await pCobrarService.getC(req.params.id); if (!row) return res.status(404).json({ error: "not_found" }); res.json(row); });
...
pCobrarRouter.get("/:id", async (req, res) => { const row = await pCobrarService.get(req.params.id); if (!row) return res.status(404).json({ error: "not_found" }); res.json(row); });
