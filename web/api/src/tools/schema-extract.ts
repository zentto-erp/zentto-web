 ...
 *     02_functions/<schema>_<func>.sql → DROP FUNCTION IF EXISTS ...
  ...
  for (const table of tablesRes.rows) {
    ...
    for (const col of colsRes.rows) {
      ...
      if (col.is_identity === "YES") {
        const gen =
          col.identity_generation === "ALWAYS" ? "ALWAYS" : "BY DEFAULT";
      ...
      colLines.push(
        `  "${col.column_name}" ${typeDef}${nullable}${defaultClause}`,
      );
    }
...
async function main(): Promise<void> {
  ...
  console.log(
    `[INFO] Conectando a ${host}:${process.env.PG_PORT ?? 5433}/${db}...`,
  );
  ...
  try {
    ...
    console.log(
      "  2. Los archivos 01_tables/ y 02_functions/ son la fuente de verdad",
    );
    console.log(
      "  3. Cualquier cambio nuevo va en migrations/postgres/NNNN_descripcion.sql",
    );
