-- =============================================
-- Stored Procedures: Cotizacion (List + Get)
-- PK: NUM_FACT nvarchar(20). Filtros: search, codigo
-- =============================================

IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'usp_Cotizacion_List')
    DROP PROCEDURE usp_Cotizacion_List
GO
...
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND name = 'usp_Cotizacion_GetByNumFact')
    DROP PROCEDURE usp_Cotizacion_GetByNumFact
GO
...
SELECT name, create_date FROM sys.objects WHERE type = 'P' AND name LIKE 'usp_Cotizacion_%';
