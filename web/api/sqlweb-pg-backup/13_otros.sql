...
DROP FUNCTION IF EXISTSteStock" a
...
DROP FUNCTION IF EXISTSrId" = v_customer_id AND "Status" <> 'VOIDED' AND "PaidFlag" = FALSE
...
DROP FUNCTION IF EXISTSo_cta AND "Fecha" < p_fecha_desde
...
DROP FUNCTION IF EXISTS      := p_descripcion,
...
DROP FUNCTION IF EXISTSO v_banco FROM "CuentasBank" WHERE "Nro_Cta" = p_nro_cta;
...
DROP FUNCTION IF EXISTSblic.sp_importar_extracto(jsonb, character varying, character varying) CASCADE;
...
DROP FUNCTION IF EXISTSE PRECISION;
...
DROP FUNCTION IF EXISTS + INTERVAL '1 day')::TIMESTAMP;
...
DROP FUNCTION IF EXISTSInicialCant", "CostoUnit"
...
DROP FUNCTION IF EXISTSs" + "Acumulado" - (
...
-- sp_movunidadesmes
DROP FUNCTION IF EXISTSblic.sp_movunidadesmes(character varying, boolean, boolean) CASCADE;
...
DROP FUNCTION IF EXISTSCerrarMesInventario(v_prev_periodo);
...
DROP FUNCTION IF EXISTS_customer_id;
END;
$function$
;
