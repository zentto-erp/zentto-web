-- Fix: usp_acct_budget_getlines - service sends p_company_id but function only takes p_budget_id
-- Add p_company_id param (ignored, for API compatibility)
DROP FUNCTION IF EXISTS public.usp_acct_budget_getlines(integer) CASCADE;
DROP FUNCTION IF EXISTS public.usp_acct_budget_getlines(integer, integer) CASCADE;
...
GRANT EXECUTE ON FUNCTION public.usp_acct_budget_getlines(integer, integer) TO zentto_app;

-- Fix: usp_acct_budget_insert - change json_array_elements alias from "r" to avoid ambiguity
...
-- Fix: usp_acct_budget_update - same json alias fix
