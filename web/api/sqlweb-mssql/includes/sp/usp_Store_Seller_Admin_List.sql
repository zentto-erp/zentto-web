-- usp_store_seller_admin_list — SQL Server 2012+
IF OBJECT_ID('dbo.usp_store_seller_admin_list', 'P') IS NOT NULL
    DROP PROCEDURE dbo.usp_store_seller_admin_list;
GO

CREATE PROCEDURE dbo.usp_store_seller_admin_list
    @CompanyId   INT,
...
BEGIN
    ...
    ;WITH ordered AS (
      SELECT s.Id, s.LegalName, s.StoreSlug, s.ContactEmail, s.TaxId, s.Status, s.CommissionRate,
             ISNULL((SELECT COUNT(*) FROM store.SellerProduct sp WHERE sp.SellerId = s.Id), 0) AS productCount,
             ISNULL((SELECT COUNT(*) FROM store.SellerProduct sp WHERE sp.SellerId = s.Id AND sp.Status = 'approved'), 0) AS approvedCount,
