...
-- 2. Crear tabla audit.AuditLog si no existe
...
-- 4. Fix: usp_sys_backoffice_tenantdetail — audit.AuditLog + ActionType
...
BEGIN
  RETURN QUERY
  SELECT
    c."CompanyId", c."CompanyCode"::VARCHAR, c."LegalName"::VARCHAR,
    c."TradeName"::VARCHAR, c."OwnerEmail"::VARCHAR,
    c."FiscalCountryCode", c."BaseCurrency",
    c."Plan"::VARCHAR, l."LicenseType"::VARCHAR, l."Status"::VARCHAR,
    l."LicenseKey"::VARCHAR, l."ExpiresAt", l."PaddleSubId"::VARCHAR,
    l."ContractRef"::VARCHAR, l."MaxUsers",
    c."CreatedAt", c."UpdatedAt",
    (SELECT COUNT(*) FROM sec."User" u WHERE u."CompanyId" = c."CompanyId" AND u."IsDeleted" = FALSE),
    (SELECT MAX(al."CreatedAt") FROM audit."AuditLog" al WHERE al."CompanyId" = c."CompanyId" AND al."ActionType" LIKE 'LOGIN%'),
    c."TenantSubdomain"::VARCHAR, c."TenantStatus"::VARCHAR
  FROM cfg."Company" c
  LEFT JOIN sys."License" l ON l."CompanyId" = c."CompanyId" AND l."Status" = 'ACTIVE'
  WHERE c."CompanyId" = p_company_id AND c."IsDeleted" = FALSE;
END;
$fn$;
-- +goose StatementEnd

-- 5. Fix: usp_sys_backoffice_tenantlist — audit.AuditLog + ActionType
-- +goose StatementBegin
...
BEGIN
  ...
  WITH filtered AS (
    SELECT
      c."CompanyId", c."CompanyCode"::VARCHAR, c."LegalName"::VARCHAR,
      c."Plan"::VARCHAR, l."LicenseType"::VARCHAR,
      l."Status"::VARCHAR AS license_status, l."ExpiresAt", c."CreatedAt",
      COUNT(DISTINCT u."UserId") AS user_count,
      MAX(al."CreatedAt") AS last_login,
      COUNT(*) OVER () AS total_count
    FROM cfg."Company" c
    LEFT JOIN sys."License" l ON l."CompanyId" = c."CompanyId" AND l."Status" = 'ACTIVE'
    LEFT JOIN sec."User" u ON u."CompanyId" = c."CompanyId" AND u."IsDeleted" = FALSE
    LEFT JOIN audit."AuditLog" al ON al."CompanyId" = c."CompanyId" AND al."ActionType" LIKE 'LOGIN%'
    WHERE c."IsDeleted" = FALSE
      AND (p_status IS NULL OR l."Status" = p_status)
      AND (p_plan IS NULL OR UPPER(c."Plan") = UPPER(p_plan))
      AND (p_search IS NULL OR c."LegalName" ILIKE '%' || p_search || '%' OR c."CompanyCode" ILIKE '%' || p_search || '%')
    GROUP BY c."CompanyId", c."CompanyCode", c."LegalName", c."Plan",
             l."LicenseType", l."Status", l."ExpiresAt", c."CreatedAt"
  )
  SELECT f."CompanyId", f."CompanyCode", f."LegalName", f."Plan", f."LicenseType",
         f.license_status, f."ExpiresAt", f."CreatedAt", f.user_count, f.last_login, f.total_count
  FROM filtered f ORDER BY f."CreatedAt" DESC LIMIT v_limit OFFSET v_offset;
END;
$fn$;
-- +goose StatementEnd
