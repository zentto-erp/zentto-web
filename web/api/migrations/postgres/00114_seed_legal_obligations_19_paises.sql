...
-- No se usa DELETE porque hr.EmployeeObligation tiene FK a LegalObligation (FK_EmployeeObligation_Obligation).
-- Se agrega UK (CountryCode, Code) si no existe para permitir ON CONFLICT.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'UQ_hr_LegalObligation_Country_Code'
      AND conrelid = 'hr."LegalObligation"'::regclass
  ) THEN
    ALTER TABLE hr."LegalObligation"
    ADD CONSTRAINT "UQ_hr_LegalObligation_Country_Code" UNIQUE ("CountryCode","Code");
  END IF;
END $$;
-- +goose StatementEnd
...
ON CONFLICT ("CountryCode","Code") DO UPDATE SET
  "Name"=EXCLUDED."Name","InstitutionName"=EXCLUDED."InstitutionName","ObligationType"=EXCLUDED."ObligationType",
  "CalculationBasis"=EXCLUDED."CalculationBasis","EmployerRate"=EXCLUDED."EmployerRate","EmployeeRate"=EXCLUDED."EmployeeRate",
  "FilingFrequency"=EXCLUDED."FilingFrequency","EffectiveFrom"=EXCLUDED."EffectiveFrom","IsActive"=EXCLUDED."IsActive",
...
ON CONFLICT ("CountryCode","Code") DO UPDATE SET
  "Name"=EXCLUDED."Name","InstitutionName"=EXCLUDED."InstitutionName","ObligationType"=EXCLUDED."ObligationType",
  "CalculationBasis"=EXCLUDED."CalculationBasis","EmployerRate"=EXCLUDED."EmployerRate","EmployeeRate"=EXCLUDED."EmployeeRate",
  "RateVariableByRisk"=EXCLUDED."RateVariableByRisk","FilingFrequency"=EXCLUDED."FilingFrequency",
  "EffectiveFrom"=EXCLUDED."EffectiveFrom","IsActive"=EXCLUDED."IsActive","UpdatedAt"=(now() AT TIME ZONE 'UTC');
-- +goose StatementEnd
...
-- +goose StatementBegin
ALTER TABLE hr."LegalObligation"
DROP CONSTRAINT IF EXISTS "UQ_hr_LegalObligation_Country_Code";
-- +goose StatementEnd
