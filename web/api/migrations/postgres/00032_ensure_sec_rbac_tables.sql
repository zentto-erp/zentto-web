-- +goose Up
-- Ensure sec RBAC tables exist in production
-- (baseline was marked as applied but these tables weren't created)
...
CREATE TABLE IF NOT EXISTS sec."ApprovalAction"(
  ...
  CONSTRAINT "FK_sec_ApprovalAction_User" FOREIGN KEY ("ActionByUserId") REFERENCES sec."User"("UserId")
);
CREATE INDEX IF NOT EXISTS "IX_sec_ApprovalAction_Request"
  ON sec."ApprovalAction" ("ApprovalRequestId");

-- +goose Down
DROP TABLE IF EXISTS sec."ApprovalAction" CASCADE;
DROP TABLE IF EXISTS sec."ApprovalRequest" CASCADE;
DROP TABLE IF EXISTS sec."ApprovalRule" CASCADE;
DROP TABLE IF EXISTS sec."PriceRestriction" CASCADE;
DROP TABLE IF EXISTS sec."UserPermissionOverride" CASCADE;
DROP TABLE IF EXISTS sec."RolePermission" CASCADE;
DROP TABLE IF EXISTS sec."Permission" CASCADE;
