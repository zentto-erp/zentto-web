...
DROP FUNCTION IF EXISTS0 OR i."IsService" = TRUE)
...
DROP FUNCTION IF EXISTSLimit", 0)
...
DROP FUNCTION IF EXISTS" ILIKE '%' || p_search || '%'
...
DROP FUNCTION IF EXISTSinteger)
 LANGUAGE plpgsql
AS $function$
...
-- usp_pos_espera_list
DROP FUNCTION IF EXISTSblic.usp_pos_espera_list() CASCADE;
...
DROP FUNCTION IF EXISTS"FiscalCorrelative"
...
DROP FUNCTION IF EXISTSp_company_id
...
DROP FUNCTION IF EXISTSLSE AND "IsActive" = TRUE
...
DROP FUNCTION IF EXISTSdiaAsset" ma ON ma."MediaAssetId" = ei."MediaAssetId"
...
DROP FUNCTION IF EXISTSFALSE AND p."IsActive" = TRUE
...
DROP FUNCTION IF EXISTSa" ILIKE '%' || p_search || '%'
...
DROP FUNCTION IF EXISTSd" = v."CompanyId" AND fc."BranchId" = v."BranchId"
...
DROP FUNCTION IF EXISTS fc."CashRegisterCode" = UPPER(v."CashRegisterCode")::character varying THEN 0 ELSE 1 END,
...
DROP FUNCTION IF EXISTS c."CustomerCode", c."CustomerName", c."FiscalId"
...
DROP FUNCTION IF EXISTS, p."DefaultTaxRate"
...
DROP FUNCTION IF EXISTSyment_method, p_fiscal_payload, p_wait_ticket_id,
...
DROP FUNCTION IF EXISTSid, p_customer_id, p_customer_code, p_customer_name, p_customer_fiscal_id,
...
DROP FUNCTION IF EXISTSW() AT TIME ZONE 'UTC',
...
CREATE OR REPLACE FUNCTION public.usp_pos_waitticketline_getitems(p_wait_ticket_id bigint)
 RETURNS TABLE(id bigint, "productoId" character varying, codigo character varying, nombre character varying, cantidad numeric, "precioUnitario" numeric, descuento numeric, iva numeric, subtotal numeric, total numeric, "supervisorApprovalId" bigint, "lineMetaJson" text)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
DROP FUNCTION IF EXISTSblic.usp_rest_admin_compra_update(integer, integer, character varying, character varying) CASCADE;
...
DROP FUNCTION IF EXISTS_compra_id;
...
DROP FUNCTION IF EXISTSId" = p_detalle_id
...
DROP FUNCTION IF EXISTSId" = p_id
...
DROP FUNCTION IF EXISTSblic.usp_rest_admin_compralinea_upsert(integer, integer, integer, character varying, numeric, numeric, numeric, numeric) CASCADE;
...
DROP FUNCTION IF EXISTSpe" = 'MASTER_PRODUCT'
...
DROP FUNCTION IF EXISTS" = mp."MenuProductId"
...
DROP FUNCTION IF EXISTSduct" p ON p."ProductId" = r."IngredientProductId"
