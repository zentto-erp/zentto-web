-- =============================================
-- Sistema de Conciliacion Bancaria - PostgreSQL
-- Tablas y Funciones
...
BEGIN
    ...
    SELECT COALESCE("Saldo", 0) INTO v_saldo_actual FROM "CuentasBank" WHERE "Nro_Cta" = p_nro_cta;
    ...
    VALUES (p_nro_cta, NOW() AT TIME ZONE 'UTC', p_tipo, p_nro_ref, p_beneficiario, p_categoria,
    ...
    IF p_documento_relacionado IS NOT NULL THEN
        ...
        VALUES (p_nro_cta, p_tipo_doc_rel, NOW() AT TIME ZONE 'UTC', v_debe, v_haber,
                p_co_usuario, p_concepto, p_documento_relacionado, v_banco, p_nro_ref);
    END IF;

    RETURN QUERY SELECT TRUE AS "ok", v_mov_id AS "movimientoId", v_saldo_actual AS "saldoNuevo";
END;
$$;
...
BEGIN
    ...
    IF v_saldo_inicial IS NULL THEN
        ...
        FROM "CuentasBank" WHERE "Nro_Cta" = p_nro_cta;
    END IF;
    ...
    IF v_saldo_final IS NULL THEN v_saldo_final := v_saldo_inicial; END IF;
    ...
    VALUES (p_nro_cta, p_fecha_desde, p_fecha_hasta,
    ...
    SELECT v_conciliacion_id, 'SISTEMA', "id", "Fecha", "Concepto", "Nro_Ref",
    ...
    RETURN QUERY SELECT v_conciliacion_id AS "conciliacionId", v_saldo_inicial AS "saldoInicial", v_saldo_final AS "saldoFinal";
END;
$$;

-- =============================================
-- 6. Funcion: Importar extracto bancario (JSONB en vez de XML)
-- =============================================
CREATE OR REPLACE FUNCTION sp_importar_extracto(
    p_extracto_json JSONB,
    p_nro_cta VARCHAR(20),
    p_co_usuario VARCHAR(60) DEFAULT 'API'
)
RETURNS TABLE (
...
DECLARE
    ...
    v_count INT := 0;
BEGIN
    FOR elem IN SELECT jsonb_array_elements(p_extracto_json)
    LOOP
    ...
    RETURN QUERY SELECT TRUE AS "ok", v_count AS "registrosImportados";
END;
$$;
...
CREATE OR REPLACE FUNCTION sp_conciliar_movimientos(
    ...
    p_movimiento_sistema_id INT,
    p_extracto_id INT DEFAULT NULL,
    p_co_usuario VARCHAR(60) DEFAULT 'API'
)
RETURNS TABLE (
  "ok" BOOLEAN,
...
BEGIN
    ...
    FROM "ConciliacionDetalle" WHERE "ID" = p_movimiento_sistema_id;
    ...
    WHERE "Conciliacion_ID" = p_conciliacion_id AND "Tipo_Origen" = 'SISTEMA' AND "Conciliado" = TRUE;

    SELECT SUM(CASE WHEN e."Tipo" = 'DEBITO' THEN e."Monto" ELSE 0 END),
    ...
    RETURN QUERY SELECT TRUE AS "ok", 'Movimiento conciliado'::VARCHAR AS "mensaje";
END;
$$;
...
BEGIN
    SELECT "Nro_Cta" INTO v_nro_cta FROM "ConciliacionBancaria" WHERE "ID" = p_conciliacion_id;
    ...
    PERFORM sp_generar_movimiento_bancario(
        p_nro_cta := v_nro_cta,
        p_tipo := v_tipo_mov,
        p_nro_ref := 'AJUSTE-' || CAST(p_conciliacion_id AS VARCHAR),
    ...
    VALUES (p_conciliacion_id, 'AJUSTE', NOW() AT TIME ZONE 'UTC', p_descripcion,
            'AJUSTE-' || CAST(p_conciliacion_id AS VARCHAR), v_debito, v_credito, TRUE, p_tipo_ajuste, p_co_usuario);

    RETURN QUERY SELECT TRUE AS "ok", 'Ajuste generado'::VARCHAR AS "mensaje";
END;
$$;
...
BEGIN
    ...
    FROM "ConciliacionBancaria" WHERE "ID" = p_conciliacion_id;
    ...
    RETURN QUERY SELECT TRUE AS "ok", v_diferencia AS "diferencia", v_estado AS "estado";
END;
$$;

-- =============================================
-- 10. Funcion: Listar conciliaciones
-- =============================================
...
BEGIN
    SELECT COUNT(1) INTO p_total_count FROM "ConciliacionBancaria"
    WHERE (p_nro_cta IS NULL OR "Nro_Cta" = p_nro_cta)
      AND (p_estado IS NULL OR "Estado" = p_estado);

    -- Nota: La query principal se ejecuta como un RETURN QUERY separado
    -- Para compatibilidad con el patron Node.js, se recomienda usar
    -- la version RETURNS TABLE a continuacion
END;
$$;

-- Version con RETURNS TABLE para compatibilidad con Node.js
CREATE OR REPLACE FUNCTION sp_conciliacion_list_v2(
    p_nro_cta VARCHAR(20) DEFAULT NULL,
    p_estado VARCHAR(20) DEFAULT NULL,
    p_page INT DEFAULT 1,
    p_limit INT DEFAULT 50
...
DECLARE
    ...
    v_total BIGINT;
BEGIN
    SELECT COUNT(1) INTO v_total FROM "ConciliacionBancaria"
    WHERE (p_nro_cta IS NULL OR "ConciliacionBancaria"."Nro_Cta" = p_nro_cta)
      AND (p_estado IS NULL OR "ConciliacionBancaria"."Estado" = p_estado);

    RETURN QUERY
    SELECT c."ID", c."Nro_Cta", c."Fecha_Desde", c."Fecha_Hasta",
           ...
           b."Banco",
           (SELECT COUNT(1) FROM "ConciliacionDetalle" d WHERE d."Conciliacion_ID" = c."ID" AND d."Conciliado" = FALSE) AS "Pendientes",
           (SELECT COUNT(1) FROM "ConciliacionDetalle" d WHERE d."Conciliacion_ID" = c."ID" AND d."Conciliado" = TRUE) AS "Conciliados",
...
-- =============================================
-- 11. Funcion: Obtener detalle de conciliacion
-- PG no puede retornar multiple result sets en una sola funcion,
-- asi que se divide en funciones separadas.
-- =============================================
...
BEGIN
    RETURN QUERY
    SELECT d."ID", d."Conciliacion_ID", d."Tipo_Origen", d."MovCuentas_ID",
...
BEGIN
    ...
    FROM "ConciliacionBancaria" cb WHERE cb."ID" = p_conciliacion_id;

    RETURN QUERY
    SELECT e."ID", e."Nro_Cta", e."Fecha", e."Descripcion", e."Referencia",
...
-- =============================================
-- 12. Funcion: Obtener movimiento bancario por ID
-- =============================================
...
BEGIN
    RETURN QUERY
    SELECT m."id", m."Nro_Cta", m."Fecha", m."Tipo", m."Nro_Ref",
           m."Beneficiario", m."Gastos", m."Ingresos", m."Saldo", m."Concepto",
           m."Confirmada",
           c."Banco" AS "BancoNombre",
           c."Descripcion" AS "CuentaDescripcion",
           c."Saldo" AS "SaldoActual"
    FROM "MovCuentas" m
    INNER JOIN "CuentasBank" c ON c."Nro_Cta" = m."Nro_Cta"
