...
DECLARE
    ...
    v_asiento_id INT;
BEGIN
    -- NOTA: La tabla legacy "Cuentas" (public) fue eliminada.
    -- El equivalente canonico es acct."Account" (ver 15_seed_contabilidad.sql).
    -- Este bloque se omite si la tabla legacy no existe.
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'Cuentas' AND table_schema = 'public' AND table_type = 'BASE TABLE') THEN
        ...
        END IF;
    ELSE
        RAISE NOTICE 'Tabla Cuentas no existe (legacy eliminada), omitida. Ver acct."Account" en 15_seed_contabilidad.sql';
    END IF;

    -- ============================================
    -- ASIENTOS DE EJEMPLO
    -- ============================================
    -- NOTA: Las tablas legacy "Asientos" y "Asientos_Detalle" (public) fueron eliminadas.
    -- El equivalente canonico es acct."JournalEntry" / acct."JournalEntryLine".
    -- Este bloque se omite si las tablas legacy no existen.
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'Asientos' AND table_schema = 'public' AND table_type = 'BASE TABLE') THEN
        ...
        END IF;
    ELSE
        RAISE NOTICE 'Tablas Asientos/Asientos_Detalle no existen (legacy eliminadas), omitidas. Ver acct."JournalEntry" / acct."JournalEntryLine"';
    END IF;
