-- ============================================================
-- Zentto PostgreSQL — Shipping Module Functions
...
BEGIN
  ...
  IF p_shipping_address_id IS NULL OR p_shipping_address_id = 0 THEN
    ...
    RETURN QUERY SELECT 1, 'Dirección creada'::VARCHAR;
  ELSE
    ...
    RETURN QUERY SELECT 1, 'Dirección actualizada'::VARCHAR;
  END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  VALUES (v_shipment_id, 'CREATED', 'DRAFT', 'Envío creado', 'CUSTOMER');
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM logistics."Shipment" WHERE "ShipmentId" = p_shipment_id) THEN
    RETURN QUERY SELECT 0, 'Envío no encontrado'::VARCHAR;
    RETURN;
  END IF;
...
BEGIN
  IF EXISTS (SELECT 1 FROM logistics."CustomsDeclaration" WHERE "ShipmentId" = p_shipment_id) THEN
    ...
    WHERE "ShipmentId" = p_shipment_id;
    RETURN QUERY SELECT 1, 'Declaración actualizada'::VARCHAR;
  ELSE
    ...
    RETURN QUERY SELECT 1, 'Declaración creada'::VARCHAR;
  END IF;
END;
$$ LANGUAGE plpgsql;
